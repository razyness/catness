...
class Search(commands.Cog):
	...
	async def search(self, inter, query: str):
		...
		embed.add_field(name=f"Status: `{data['attributes']['status'].capitalize()}`",
						value=f"Created <t:{int(datetime.fromisoformat(data['attributes']['createdAt']).timestamp())}:R>\n"
							  f"Updated <t:{int(datetime.fromisoformat(data['attributes']['updatedAt']).timestamp())}:R>")

		tags = ", ".join(
			f"[`{tag['attributes']['name']['en']}`](https://mangadex.org/tag/{tag['id']})" for tag in
