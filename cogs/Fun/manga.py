...
async def get_chapters(manga_id, languages=['en']):
...
async def get_page_url(manga_id, chapter, page):
...
async def get_pages_count(chapter):
	...
	r = await http.get(f"{base_url}/{chapter}")
	return len(r["chapter"]["data"])

http = http.HTTP()
class ViewThing(discord.ui.View):
	def __init__(self, *, timeout: float | None = 180, manga_id, manga_title):
	...
	async def read(self, inter:discord.Interaction, button):
		...
		embed.set_author(name=self.manga_title, url=f"https://mangadex.org/title/{self.manga_id}")

		view = Reader(manga_id=self.manga_id, manga_title=self.manga_title, chapters=chapters, pages=pages)
...
class Reader(discord.ui.View):
	...
	async def back(self, inter, button):
		...
		embed = discord.Embed(title=f"Chapter {self._chapter + 1}: {self._chapter_list[self._chapter][1]}")
		embed.set_image(url=page_url)
		embed.set_footer(text=f"Ch.{self._chapter + 1}/{len(self._chapter_list)}  •  Pg.{self._page + 1}/{self._pages}")
		embed.set_author(name=self.manga_title, url=f"https://mangadex.org/title/{self.manga_id}")
...
class Search(commands.Cog):
	...
	async def search(self, inter, query: str):
		...
		stats = await http.get(f"https://api.mangadex.org/statistics/manga/{data['id']}")
		stats = stats['statistics'][data['id']]
		...
		view = ViewThing(manga_id=data['id'], manga_title=data["attributes"]["title"]["en"])
		await inter.response.send_message(embed=embed, view=view, ephemeral=True)


async def setup(ce):
	await ce.add_cog(Search(ce))
