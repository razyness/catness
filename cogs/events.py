...
class Events(commands.Cog):
	def __init__(self, bot: Bot):
		self.bot = bot
		self.tree = bot.tree
		self.blocked = ["cogs.events", "cogs.fun.openapi", 'cogs.others.error_handler', 'cogs.fun.manga']
	...
	async def on_ready(self):
		...
		print(f"🟩 Logged in as {self.bot.user} with a {round(self.bot.latency * 1000)}ms delay")
	...
	async def on_guild_join(self, guild):
		async with self.bot.db_pool.acquire() as conn:
			try:
				...
				print(f"📥 I have joined the guild {guild.name} | {guild.id}, and added it to my database")
	...
	async def on_guild_remove(self, guild):
		async with self.bot.db_pool.acquire() as conn:
			try:
				...
				print(f"📤 I have left the guild {guild.name} | {guild.id}, and removed it from my database")
			except asyncpg.exceptions.PostgresError as e:
				print(f"🟥 I could not remove the guild {guild.name} | {guild.id} from my database:", e)
