...
class DownloadEmotes(ui.View):
	...
	async def download(self, inter: discord.Interaction, button: discord.ui.Button):
		...
		self.saved = {}
		await message.edit(content="Downloading... `100%`\nZipping... This may take a while\nUploading...")
		await inter.followup.send(content=inter.user.mention,
								  file=discord.File(zip_file, f'{self.guild.id}_emotes.zip'), ephemeral=True)
...
class ServerView(ui.View):
	...
	async def members_button(self, interaction: discord.Interaction, button: discord.ui.Button):
		if self.guild.member_count > 1000:
			conf = await confirm.send(interaction, "Are you sure?",
									  f"This server has `{self.guild.member_count}` members. Get ready for some spam!")
			if not conf:
				return
	...
	async def emojis_button(self, interaction: discord.Interaction, button: discord.ui.Button):
		if len(self.guild.emojis) > 250:
			conf = await confirm.send(interaction, "Are you sure?",
									  f"This server has `{len(self.guild.emojis)}` emojis. Get ready for some spam!")
			if not conf:
				return
...
class ServerInfo(commands.Cog):
	...
	async def serverinfo(self, inter: discord.Interaction, ephemeral: bool = True):
		...
		if guild.splash:
			view.add_item(
				discord.ui.Button(row=1, label="Invite splash", style=discord.ButtonStyle.url, url=guild.splash.url))
		if guild.discovery_splash:
			view.add_item(discord.ui.Button(row=1, label="Discovery splash", style=discord.ButtonStyle.url,
