	...
	async def snipe(self, inter):
		...
		if not snipe_data.attachments and not snipe_data.stickers:
			...
			for page in (snipe_data.attachments + snipe_data.stickers):
				...
				if isinstance(page, discord.Attachment) and Path(urlparse(page.url).path).suffix not in ['.png', '.jpg',
																										 '.jpeg',
																										 '.gif',
																										 '.webp']:
			...
			if Path(urlparse(snipe_data.attachments[0].url).path).suffix not in ['.png', '.jpg', '.jpeg', '.gif',
																				 'webp']:
