	...
	async def rep(self, inter, user: discord.User):
		...
		async with self.bot.db_pool.acquire() as connection:
			async with connection.transaction():
				...
				if muser is None:
					await connection.execute("INSERT INTO profiles (id, rep_value, rep_time) VALUES ($1, 0, 0)",
											 user.id)
					muser = await connection.fetchrow("SELECT * FROM profiles WHERE id = $1", user.id)
				...
				if expiration_time < datetime.datetime.utcnow() or inter.user.id == 809275012980539453:
					...
					await connection.execute("UPDATE profiles SET rep_time = $1 WHERE id = $2", int(time.time()),
											 inter.user.id)
					await inter.response.send_message(f"You gave {user.mention} a reputation point!!")
				else:
					await inter.response.send_message(
						f"You don't have any reputation points left! Return <t:{int(datetime.datetime.timestamp(expiration_time))}:R>",
						ephemeral=True)
