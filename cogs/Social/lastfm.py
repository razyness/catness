		...
		if "error" in now_playing:
			...
			for i in now_playing["recenttracks"]["track"][0]:
				if i == "@attr":
					...
					status = ""
					embed.set_footer(
						text=f'{embed.footer.text}- {to_relative(now_playing["recenttracks"]["track"][0]["date"]["uts"])}')
					break
			embed.set_thumbnail(
...
async def overview(user, session):
	...
	if user_data["user"]["subscriber"] == "1":
		...
		embed.title = f'{user_data["user"]["realname"]}'
		footer_thing = ''
	embed.set_thumbnail(url=user_data["user"]["image"][2][
								"#text"] or 'https://lastfm.freetls.fastly.net/i/u/avatar170s/818148bf682d429dc215c1705eb27b98.png')
		...
		if user is None or user.startswith("<@"):
			...
			try:
				...
				await interaction.response.send_message(
					f"{g} linked a `LastFM` account! Run </link:1080264642569441380> to do so", ephemeral=True)
				return
